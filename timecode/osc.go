...
func ListenOSC(ctx context.Context, address string, coordinator *Coordinator) error {
	...
	defer stopClose()
	buffer := make([]byte, oscReadBufferSize)
	for {
		...
		if err == nil {
			if err := coordinator.Update(SourceOSC, position, true); err != nil {
				return err
			}
		}
	}
...
func ParseOSC(packet []byte, frameRate float64) (time.Duration, error) {
	...
	case 's':
		value, _, err := osc.ReadString(packet, offset)
		if err != nil {
			return 0, err
		}
