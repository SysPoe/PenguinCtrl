...
func TestTimecodeMarkerInputsPreserveMarkerValues(t *testing.T) {
	...
	cue.Play.Sound.Timecode = []show.TimecodeMarker{
		{
			...
			Action: show.CuePlay{MediaControl: &show.MediaControlPlay{
		...
		{
			TimeMs: 3750,
			Type:   show.CueTypeRemote,
			Action: show.CuePlay{Remote: &show.RemotePlay{
	...
	if fields[0].time.Value != 1250 || fields[0].disabled.Checked || fields[0].actionType.Selected != timecodeActionIndex(show.CueTypeMediaControl) {
...
func TestResetTimecodeInputsRebuildsTypedMarkerState(t *testing.T) {
	cue := show.NewSoundCue()
	cue.Play.Sound.Timecode = []show.TimecodeMarker{
		{TimeMs: 100, Type: show.CueTypeRemote, Action: show.NewRemoteCue().Play},
		{TimeMs: 200, Type: show.CueTypeOutputControl, Action: show.NewOutputControlCue().Play},
	}
